...
// Inicialización de IndexedDB (mínima)
...
function showDetail(id) {
  ...
  content.innerHTML = `
    <div class="screenshots">${(game.screenshots||[]).map(s=>`<img src="${s}" alt="screenshot">`).join('')}</div>
...
async function toggleFavorite(id) {
  ...
  await syncFavorites();
  if (document.getElementById('home').classList.contains('active')) renderGames(currentGenre, document.getElementById('searchInput')?.value || '');
...
async function syncDownload(gameId) {
  if (!userToken) return;
  await apiCall('/api/sync', 'POST', { downloads: [{gameId, deviceId, deviceName: getDeviceName(), deviceInfo: getDeviceInfo()}] });
}
...
async function loadSyncFromServer() {
  ...
  try {
    ...
    // Las descargas remotas se muestran en la pantalla de dispositivos
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    alert('Error al cargar el catálogo. Comprueba tu conexión.');
  }
  ...
  if (userToken) {
    try {
      await apiCall('/api/me'); // verifica token
  ...
  try {
    ...
    for (const [id, info] of Object.entries(devices)) {
      ...
      entry.innerHTML = `
        ...
        <div class="downloads">
          ${(info.downloads||[]).map(d => `<span class="dl-chip">${games.find(g=>g.id===d.gameId)?.title || 'Juego '+d.gameId}</span>`).join('')}
        </div>
      `;
      list.appendChild(entry);
    }
    document.getElementById('editNameBtn')?.addEventListener('click', ()=> document.getElementById('deviceNameDialog').show());
  } catch (e) {
